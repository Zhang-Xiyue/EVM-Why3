...
type memory = (int) -> (memory_content option)
...
let dup_inst_opcode (inst: int) : Z.t = Z.add (inst) (Z.of_string "128")

let dup_inst_num (inst: int) : (Z.t) * (Z.t) = (inst, Z.add (inst) Z.one)

type swap_inst = int

let swap_inst_opcode (inst: int) : Z.t = Z.add (inst) (Z.of_string "144")

let swap_inst_num (inst: int) : (Z.t) * (Z.t) =
...
type machine_state = {
  mac_stack: (int) list;
  mac_memory: (int) -> (memory_content option);
  mac_pc: Z.t;
  mac_status: vmstatus;
...
let update_memory_content (m: (int) -> (memory_content option)) (idx: int)
                          (cont: memory_content option) : (int) -> (memory_content option)
  =
  fun (addr: int) -> if addr == idx then begin cont end else begin m addr end

let push_stack (s: (int) list) (ele: int) : (int) list = ele :: s

let pop_stack (s: (int) list) : ((int) list) * ((int) option) =
  begin match s with
  | x :: t -> (t, Some x)
  | _ -> (s, None)
  end
...
let update_memory (memo: (int) -> (memory_content option)) (m: machine_state) : machine_state
  =
...
let interpreter (m: machine_state) : machine_state =
  ...
  | Some Arith ADD ->
    ...
    | (Some aqt, Some bqt) ->
      { mac_stack = (push_stack stqtqt (aqt + bqt)); mac_memory =
  ...
  | Some Arith MUL ->
    ...
    | (Some aqt, Some bqt) ->
      { mac_stack = (push_stack stqtqt1 (aqt * bqt)); mac_memory =
        (m.mac_memory); mac_pc = (Z.add (m.mac_pc) Z.one); mac_status =
        (m.mac_status); mac_memory_usage = (m.mac_memory_usage); mac_gas =
        (m.mac_gas); mac_insts = (m.mac_insts); mac_jumpmap =
        (m.mac_jumpmap) }
  ...
  | Some Arith SUB ->
    ...
    | (Some aqt, Some bqt) ->
      { mac_stack = (push_stack stqtqt2 (aqt - bqt)); mac_memory =
        (m.mac_memory); mac_pc = (Z.add (m.mac_pc) Z.one); mac_status =
        (m.mac_status); mac_memory_usage = (m.mac_memory_usage); mac_gas =
        (m.mac_gas); mac_insts = (m.mac_insts); mac_jumpmap =
        (m.mac_jumpmap) }
  ...
  | Some Arith DIV ->
    ...
    | (Some aqt, Some bqt) ->
      { mac_stack = (push_stack stqtqt3 (aqt / bqt)); mac_memory =
        (m.mac_memory); mac_pc = (Z.add (m.mac_pc) Z.one); mac_status =
        (m.mac_status); mac_memory_usage = (m.mac_memory_usage); mac_gas =
        (m.mac_gas); mac_insts = (m.mac_insts); mac_jumpmap =
        (m.mac_jumpmap) }
